// COSTA carta V2 — render PDF + preview screenshots
...
(async () => {
  ...
  // Feature dish — lubina
  await mobilePage.evaluate(() => {
    const target = document.querySelector('.feature-dish');
    if (target) target.scrollIntoView({ behavior: 'instant', block: 'start' });
  });
  await mobilePage.waitForTimeout(400);
  await mobilePage.screenshot({
    path: path.resolve(__dirname, 'preview/mobile-04-lubina.png'),
    fullPage: false,
  });
  ...
  // -----------------------------------------------------------
  // 2. PDF imprimible — A3 horizontal gatefold triple
  // -----------------------------------------------------------
