// TU CAPRICHO carta — render PDF + preview screenshots
...
(async () => {
  ...
  await mobilePage.evaluate(() => {
    const target = document.querySelector('.acreditaciones');
    if (target) target.scrollIntoView({ behavior: 'instant', block: 'start' });
  });
  await mobilePage.waitForTimeout(400);
  await mobilePage.screenshot({
    path: path.resolve(__dirname, 'preview/mobile-03-acreditaciones.png'),
    fullPage: false,
  });
  ...
  await mobilePage.waitForTimeout(400);
  await mobilePage.screenshot({
    path: path.resolve(__dirname, 'preview/mobile-04-arroces.png'),
    fullPage: false,
  });

  // Tapas
  await mobilePage.evaluate(() => {
    const target = document.querySelector('#tapas');
    if (target) target.scrollIntoView({ behavior: 'instant', block: 'start' });
  });
  await mobilePage.waitForTimeout(400);
  await mobilePage.screenshot({
    path: path.resolve(__dirname, 'preview/mobile-05-tapas.png'),
    fullPage: false,
  });
