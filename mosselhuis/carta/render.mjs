// MOSSELHUIS carta — render PDF + preview screenshots
...
(async () => {
  ...
  await mobilePage.evaluate(() => {
    const target = document.getElementById('variedades');
    if (target) target.scrollIntoView({ behavior: 'instant', block: 'start' });
  });
  await mobilePage.waitForTimeout(400);
  await mobilePage.screenshot({
    path: path.resolve(__dirname, 'preview/mobile-03-variedades.png'),
    fullPage: false,
  });

  // Hero — Marinière
  await mobilePage.evaluate(() => {
    const target = document.querySelector('.hero-dish');
    if (target) target.scrollIntoView({ behavior: 'instant', block: 'start' });
  });
  await mobilePage.waitForTimeout(400);
  await mobilePage.screenshot({
    path: path.resolve(__dirname, 'preview/mobile-04-mariniere.png'),
    fullPage: false,
  });
  ...
  await mobilePage.waitForTimeout(400);
  await mobilePage.screenshot({
    path: path.resolve(__dirname, 'preview/mobile-05-bebidas.png'),
    fullPage: false,
  });
  ...
  // -----------------------------------------------------------
  // 2. PDF imprimible — A3 horizontal gatefold (420 x 297 mm)
  // -----------------------------------------------------------
  ...
  // preferCSSPageSize: true respeta @page del CSS y evita 4 paginas.
  await printPage.pdf({
