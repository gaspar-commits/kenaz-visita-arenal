// CACAO carta — render PDF + preview screenshots
...
(async () => {
  ...
  await mobilePage.waitForTimeout(400);
  await mobilePage.screenshot({
    path: path.resolve(__dirname, 'preview/mobile-04-mojito.png'),
    fullPage: false,
  });
  ...
  await mobilePage.waitForTimeout(400);
  await mobilePage.screenshot({
    path: path.resolve(__dirname, 'preview/mobile-05-picar.png'),
    fullPage: false,
  });

  // Tostadas
  await mobilePage.evaluate(() => {
    const target = document.querySelector('.section--alt');
    if (target) target.scrollIntoView({ behavior: 'instant', block: 'start' });
  });
  await mobilePage.waitForTimeout(400);
  await mobilePage.screenshot({
    path: path.resolve(__dirname, 'preview/mobile-06-tostadas.png'),
    fullPage: false,
  });
